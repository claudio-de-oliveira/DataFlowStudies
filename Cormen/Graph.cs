...
namespace Cormen
{
    ...
    public class LNode
    {
        ...
        {
            this.vertex = vertex;
            pred = new ArrayOf<LNode>();
            succ = new ArrayOf<LNode>();
        }
        ...
        { get { return vertex; } }
        public ArrayOf<LNode> Successor
        { get { return succ; } }
        public ArrayOf<LNode> Predecessor
        { get { return pred; } }
    ...
    {
        protected ArrayOf<LNode> nodes;
        string key;

        public LGraph(string key)
        {
            nodes = new ArrayOf<LNode>();
            this.key = key;
        }
        ...
        public LGraph StronglyConnectedComponents()
        {
            ...
            //
            ArrayOf<KeyedData>[] vertex = new ArrayOf<KeyedData>[forest.Length];

            // Primeiro particiona o conjunto de vertices
            for (int t = 0; t < forest.Length; t++)
            {
                string key = "";
                vertex[t] = new ArrayOf<KeyedData>();
        ...
        public ArrayOf<KeyedData> DFS()
        {
            int time = 0;

            ArrayOf<KeyedData> tSort = new ArrayOf<KeyedData>();
        ...
        public ArrayOf<KeyedData> DFS(ArrayOf<KeyedData> Q)
        {
            int time = 0;
        ...
        private void DFS_VISIT(LNode u, ref int time, ref ArrayOf<KeyedData> tSort)
        {
            u.color = COLOR.GRAY;
            time++;
            u.d = time;
        ...
        public LGraph[] DFForest(ArrayOf<KeyedData> Q)
        {
            bool[] root = new bool[Count];

            ArrayOf<KeyedData> tSort = DFS(Q);
