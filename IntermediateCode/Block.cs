...
namespace AbstractMachine
{
    public class Block : KeyedData
    {
        ...
        public Block(int leader)
        {
            ...
            Visited = false;
            dfn = -1;
            childs = new ArrayOf<Block>();

            this.leader = leader;
            code = new IntermediateCode();
    ...
    public class GraphOfBlock : LGraph
    {
        ...
        // 
        //         this[blk].Successor.RemoveAll();
        //         this[blk].Predecessor.RemoveAll();
        // 
    ...
    public class ArrayOfBlock : ArrayOf<Block>
    {
...
//        }
        ...
        private void runDFS(Block u, BlockState[] state, Queue Q)
        {
        ...
        public string ToCode(Queue Q)
        {
            ...
            Stack<Block> stk = new Stack<Block>();
            foreach (Block block in Q)
                stk.Push(block);
            ...
            stk.Pop(); // Descarta o primeiro nó
            ...
            while (stk.Count > 0)
            {
                ...
                for (int i = 0; i < block.Size; i++)
                {
                    Console.WriteLine(block.Code[i]);
                    if (i == 0)
                        str += j + ": & $@" + block.Id + "$ & ";
                    else
                        str += j + ": & & ";
