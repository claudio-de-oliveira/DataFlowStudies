    ...
    public class BitSet
    {
        ...
        public bool Add(object obj)
        {
            for (int pos = 0; pos < universe.Count; pos++)
            {
                ...
                if ((universe[pos] == null) || (obj == null))
                    return false;

                if (universe[pos].GetType() != obj.GetType())
                    return false;

                if (universe[pos].Equals(obj))
                {
                    if (this[pos])
                        return false;
                    this[pos] = true;
